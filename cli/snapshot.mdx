---
title: "snapshot"
description: "Capture a screenshot and HTML analysis of the current page using an AI model."
---
...
<ParamField path="--page" type="string">
  Target a specific page ID within the session. Use `npx libretto pages --session <name>` to list page IDs when a popup or new tab is open. See [pages & close](/cli/session-management) for details.
</ParamField>
...
<CardGroup cols={2}>
  <Card title="exec" icon="code" href="/cli/exec">
    Run Playwright code to prototype interactions after using snapshot to identify elements.
  </Card>
  <Card title="run & resume" icon="play" href="/cli/run-and-resume">
